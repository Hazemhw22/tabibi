...
function LoginDoctorForm() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (result && !result.ok) {
        toast.error(t("auth.errors.conn_error"));
        return;
      }
  ...
  return (
    <Card className="w-full max-w-md shadow-xl border-0">
      ...
      <CardHeader className="text-center pb-2">
        <CardTitle className="text-2xl">{t("auth.doctor_login.title")}</CardTitle>
        <CardDescription className="text-base mt-1">{t("auth.doctor_login.desc")}</CardDescription>
      </CardHeader>
      <CardContent className="pt-2">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <Input
            label={t("auth.doctor_login.email")}
            type="email"
          ...
          <div className="w-full">
            <label className={authLabelClass}>{t("auth.doctor_login.password")}</label>
            ...
            <div className="text-left">
              <Link href="/forgot-password/doctor" className="text-sm text-blue-600 dark:text-blue-400">
                {t("auth.doctor_login.forgot_password")}
              </Link>
            </div>
          </div>
          <Button type="submit" className="w-full" size="lg" disabled={loading}>
            {loading ? <><IconLoader className="h-4 w-4 animate-spin" /> {t("auth.doctor_login.submitting")}</> : t("auth.doctor_login.submit")}
          </Button>
        </form>
        <div className="mt-6 text-center">
          <p className="text-base text-gray-600 dark:text-slate-400">
            {t("auth.doctor_login.no_account")}{" "}
            <Link href="/register/doctor" className="font-semibold text-blue-600 dark:text-blue-400">
              {t("auth.doctor_login.register")}
            </Link>
          </p>
