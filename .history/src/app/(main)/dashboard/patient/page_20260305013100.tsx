                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex gap-3">
                        <div className="w-12 h-12 rounded-xl bg-blue-100 flex items-center justify-center text-xl font-bold text-blue-600 shrink-0">
                          {(apt.doctor?.User?.name ?? apt.doctor?.user?.name)?.charAt(0) || "D"}
                        </div>
                        <div>
                          <h3 className="font-bold text-gray-900">
                            د. {apt.doctor?.User?.name ?? apt.doctor?.user?.name}
                          </h3>
                          <p className="text-sm text-blue-600">
                            {apt.doctor?.Specialty?.nameAr ?? apt.doctor?.specialty?.nameAr}
                          </p>
                        </div>
                      </div>
                    ...
                    return (
                      <tr key={apt.id} className="hover:bg-gray-50 transition-colors">
                        <td className="py-3 pr-4">
                          <div>
                            <p className="font-medium text-gray-900">د. {apt.doctor?.User?.name ?? apt.doctor?.user?.name}</p>
                            <p className="text-xs text-gray-500">{apt.doctor?.Specialty?.nameAr ?? apt.doctor?.specialty?.nameAr}</p>
                          </div>
                        </td>
                        ...
                        <td className="py-3 font-semibold text-gray-900">₪{apt.fee}</td>
                        <td className="py-3">
                          {apt.status === "COMPLETED" && (!apt.Review || (Array.isArray(apt.Review) && !apt.Review.length)) && (
